module.exports = {
  // Wpisz ID swojego serwera Discord
  GUILD_ID: "WSTAW_ID_SERWERA",

  // Kanał, na którym bot wysyła powitania
  WELCOME_CHANNEL_ID: "WSTAW_ID_KANALU_POWITAN",

  // Kanał, do którego bot wysyła logi
  LOG_CHANNEL_ID: "WSTAW_ID_KANALU_LOGOW",

  // Rola nadawana po kliknięciu weryfikacji
  VERIFIED_ROLE_ID: "WSTAW_ID_ROLI",

  // Kategoria, w której mają powstawać tickety
...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption(o => o.setName("powod").setDescription("Powód")),
...
client.on("guildMemberAdd", async member => {
  if (isConfigured(config.WELCOME_CHANNEL_ID)) {
    ...
    if (channel) {
      const embed = new EmbedBuilder()
        ...
        .setDescription(`Witaj ${member}! Miło Cię widzieć na **${member.guild.name}**.`)
  ...
  await sendLog(member.guild, "📥 Nowy użytkownik", `${member.user.tag} dołączył na serwer.`);
});

client.on("guildMemberRemove", async member => {
  await sendLog(member.guild, "📤 Użytkownik wyszedł", `${member.user.tag} opuścił serwer.`);
});

client.on("messageDelete", async message => {
  if (!message.guild || message.author?.bot) return;
  const content = message.content?.slice(0, 1000) || "(brak treści)";
  await sendLog(
    message.guild,
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === "ticket") {
        const embed = new EmbedBuilder()
          ...
          .setDescription("Potrzebujesz pomocy? Kliknij przycisk, aby utworzyć ticket.");
      ...
      if (interaction.commandName === "ban") {
        ...
        return sendLog(interaction.guild, "🔨 Ban", `**${user.tag}** został zbanowany przez ${interaction.user.tag}.\nPowód: ${reason}`);
      }
      ...
      if (interaction.commandName === "kick") {
        ...
        return interaction.reply(`👢 **${user.tag}** został wyrzucony. Powód: ${reason}`);
      }
      ...
      if (interaction.commandName === "mute") {
        ...
        const member = await interaction.guild.members.fetch(user.id).catch(() => null);
        if (!member) return interaction.reply({ content: "❌ Nie znaleziono użytkownika.", ephemeral: true });
        await member.timeout(10 * 60 * 1000, `Mute przez ${interaction.user.tag}`);
        return interaction.reply(`🔇 **${user.tag}** został wyciszony na 10 minut.`);
      }
      ...
      if (interaction.commandName === "unmute") {
        ...
        return interaction.reply(`🔊 **${user.tag}** może już pisać.`);
      }
    ...
    if (interaction.isButton()) {
      if (interaction.customId === "verify") {
        if (!isConfigured(config.VERIFIED_ROLE_ID)) {
          return interaction.reply({ content: "⚠️ Administrator nie ustawił jeszcze VERIFIED_ROLE_ID.", ephemeral: true });
        }

        const role = interaction.guild.roles.cache.get(config.VERIFIED_ROLE_ID);
        if (!role) return interaction.reply({ content: "❌ Nie znaleziono roli weryfikacyjnej.", ephemeral: true });

        await interaction.member.roles.add(role);
        return interaction.reply({ content: "✅ Zostałeś zweryfikowany!", ephemeral: true });
      }
      ...
      if (interaction.customId === "create_ticket") {
        ...
        const embed = new EmbedBuilder()
          .setColor(config.EMBED_COLOR)
          .setTitle("🎫 Ticket")
          .setDescription("Opisz swój problem. Administracja odpowie tak szybko, jak będzie mogła.");
        ...
        return interaction.reply({ content: `✅ Utworzono ticket: ${channel}`, ephemeral: true });
      }
...
if (!process.env.DISCORD_TOKEN) {
  console.error("Brak DISCORD_TOKEN. Ustaw zmienną środowiskową DISCORD_TOKEN.");
  process.exit(1);
}
